...
var Listener = DNSListener{cache: cache.DefaultDNSCache}
...
// USE PATH PARAM
func StartDNSListener(monitorPath string) error {
...
func readMonitorDNSPackets(ctx context.Context, conn net.Conn, packets chan *monitor.Dissection) {
	// dissect
...
func (l *DNSListener) listenForDNSPackets(ctx context.Context, conn net.Conn) {
	...
	packets := make(chan *monitor.Dissection, 1024)
	readerCtx, cancel := context.WithCancel(ctx)
	defer cancel()
	go readMonitorDNSPackets(readerCtx, conn, packets)

	var packet *monitor.Dissection
	for !isDone(ctx) {
		select {
		case <-ctx.Done():
			return
		...
		l.cache.Update("agent", &packet.DNS)
		for i := range packet.DNS.Answers {
			if packet.DNS.Answers[i].Type != layers.DNSTypeA || packet.DNS.Answers[i].IP == nil {
...
func readDNS(data []byte) (*monitor.Dissection, error) {
	dissect, _, err := monitor.GetDissect(data)
	switch {
