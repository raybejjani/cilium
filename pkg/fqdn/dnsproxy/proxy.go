...
// Note: All public fields are read only and no not require locking
type DNSProxy struct {
	lock.Mutex // only allowed is protected by this

	// BindAddr is the local address the server us using to listen for DNS
...
// NotifyOnDNSResponseFunc handle propagating DNS response data
// See DNSProxy.LookupEndpointIDByIP for usage.
...
// The proxy will listen on address and port. Address may be empty, port must be non-zero.
