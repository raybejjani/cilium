...
type AddOptions struct {
	...
	// If the current revision in the policy repository doesn't match this,
	// return an error. 0 is considered "do-not-care"
	ExpectedRevision uint64
}

var policyAddBadRevision = errors.New("ExpectedRevision mismatch in daemon.PolicyAdd")
...
func (d *Daemon) PolicyAdd(rules policyAPI.Rules, opts *AddOptions) (uint64, error) {
	...
	d.policy.Mutex.Lock()
	// Guard against races on PolicyAdd by different components. We rely on the
	// policy revision only changing here atomically.
	if oldRev := d.policy.GetRevision(); opts != nil && opts.ExpectedRevision != 0 && opts.ExpectedRevision != oldRev {
		d.policy.Mutex.Unlock()
		return oldRev, policyAddBadRevision
	}
	...
	//var removedFQDNRules []*policyAPI.Rule
	if opts != nil {
		if opts.Replace {
			for _, r := range rules {
				...
				if len(oldRules) > 0 {
					...
					//removedFQDNRules = append(removedFQDNRules, oldRules...)
				}
			}
		}
	...
	//d.dnsRuleGen.StopManageDNSName(removedFQDNRules)
